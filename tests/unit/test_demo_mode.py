...
const selected=samples.render([raw,'Another output'],{selected:1,highlights:['<script>'],judgmentDetails:true,judgments:[{sample_index:1,classification:'COMPLIES',rationale:'<img src=x>'}]});
assert.ok(selected.includes('data-recorded-sample="1" aria-pressed="true"'));
assert.ok(selected.includes('<mark>&lt;script&gt;</mark>'));
assert.ok(selected.includes('Recorded LLM judgment: COMPLIES'));
assert.ok(selected.includes('&lt;img src=x&gt;'));
assert.ok(!selected.includes('<script>'));assert.ok(!selected.includes('<img'));
const compare=require('./static/js/order_comparison.js');
