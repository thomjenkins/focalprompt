...
def test_workspace_v2_contains_scenarios_and_v1_migration():
    js = (REPO / 'static' / 'js' / 'app.js').read_text(encoding='utf-8')
    assert 'WORKSPACE_SESSION_VERSION = 2' in js
    assert 'migrateWorkspaceV1' in js
    assert 'scenario: readMainScenario()' in js
