...
The [Jev experiment](docs/jev-experiment.md) selects relevant foci and optionally reorders them. Use **Analyse this prompt** under either composed prompt to open the full analysis in a separate lab tab, with retained chat, remapped focus spans and generation settings ready. Begin at step 2 with fresh predictions and outputs; the original experiment stays available. You can also download the derived analysis workspace for later import.

[Focus-order sensitivity](docs/focus-order-workflow.md) samples and judges outputs in separate requests, retains completed work after a failure, and resumes missing work. Export the workspace to preserve an incomplete run across reloads.

Step 9, [baseline + singleton focus analysis](docs/singleton-analysis.md), compares **no foci → each focus individually → all foci → each focus removed**. It reports influence from singleton/no-focus comparisons, normalized sufficiency toward full-prompt behavior, and unchanged leave-one-out necessity. Negative sufficiency is retained; normalized metrics become unavailable when full/no-focus contrast is too small relative to numerical or sampling noise. The mode reuses matching saved samples, resumes partial runs, and exports all four conditions. It also works on Jev-derived analysis workspaces.

Its pairwise grid compares outputs with the singleton patterns for each pair of foci, using only full/ablated contexts retaining both. Separate full-prompt, eligible-ablation and combined views show closer-to-row shares and mean similarity gaps; selecting a cell reveals counts, distances and actual outputs. Shared sample pools count once, and conditions receive equal weight. These are descriptive similarities, not causal dominance or quality scores. Older completed runs can build the grid from saved outputs using embeddings only.
