...
- **`utils/span_alignment.py`.** Grounds each focus to `[char_start, char_end)` in the original prompt. Matching order: exact → whitespace/quote-normalised → `evidence_quote` (unique) with optional expansion to the enclosing sentence/line → conservative token alignment for formatting-only drift. Ambiguous multi-matches stay unverified. Verified foci overwrite `prompt_section` with `prompt[char_start:char_end]` so ablation never deletes paraphrased text. `compute_coverage_report` exposes coverage %, uncovered spans, overlaps, and unverified proposals. `classify_foci_for_ablation` still sets `attributable` / `reason` / `overlap_with`. `delete_span` removes the half-open interval and optionally collapses a doubled blank line *at the join only*.
- **`services/assessment_service.py` `detect_foci`.** Asks the LLM for conceptual `focus`, verbatim `evidence_quote`, and optional `prompt_section`, then runs deterministic grounding. Unverified foci keep null offsets and are never ablated. Response includes a `coverage` report.
