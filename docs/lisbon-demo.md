...
The opening scenario automatically scrolls the two original instruction editors to Appointment booking and Cat only, using their saved source spans and the current text wrapping. Returning, resuming or resetting the guide restores these positions; readers can still scroll manually or explore the workspace.

The compact footer contains navigation and layout controls without commentary. Spotlight selects two ablation comparisons and three singleton conditions; Explore exposes every recorded focus and arm. No outputs are regenerated or omitted. The only changes to recorded text are the disclosed clinic-location redactions described below.
...
The **Focus order** stop drives the same recorded-permutation comparator available in the lab. Condition A is **Relevance → Cat only → Opening hours → Address** (shuffle #4), with 1/3 outputs judged compliant; condition B is **Address → Cat only → Relevance → Opening hours** (shuffle #2), with 0/3 compliant. The guide shows them in this order. They are selected by exact `ordered_focus_names`, cross-checked against `focus_positions`, source text, model, sampling settings and role. An absent or inconsistent condition fails visibly, with no substituted sweep result. Cat only stays in the same physical second slot; the other three existing card nodes animate around it. Reduced-motion preferences disable the animation. The headline counts literal mentions of “cat-only clinic” in the recorded outputs: **3/3 acknowledge cat-only** versus **0/3**. The original **Recorded LLM judge: 1/3 compliant** and **0/3 compliant** scores appear separately. The shared output browser opens output 2 for condition A and output 1 for condition B, with all three original samples selectable, literal phrase highlights, expandable recorded judgments and exact exported JSON. Returning or resuming restores those defaults. **Compare other permutations** exposes native selectors; **Full order experiment** retains all global results and the four-position controlled sweep. Returning to the guide restores the matched comparison.
...
The prompt view emphasizes the stronger booking instruction and added hierarchy focus, while retaining subdued original context and the cat-only instruction. “must always” is visually uppercased; the underlying stored text is unchanged. Baseline uses the shared output browser, with sample 1 initially selected and all ten samples inspectable. The matrix preselects booking (row) versus Cat-only (column), using **Full + eligible ablations** and the existing color/share encoding. The selected cell remains below sticky column headings even at 1280×720. No new dominance score or inference request is introduced.

The Jev **Ordering** stop shows the two saved composed prompts side by side: source order and Jev order. The shared lab inspector traces each source focus with the same colour and original focus number, from an ordered chip sequence into the joined message text. Highlights use the saved composition spans, including overlaps; they do not guess from matching text. Retained chat stays separate, unlabelled text is preserved, and the exact messages and output contract remain expandable. Only blank spacing is compressed visually. A missing or inconsistent span mapping falls back to the exact raw messages.
...
| Global A: Relevance → Cat only → Opening hours → Address | 1/3 COMPLIES; all mention cat-only, output 2 redirects toward a different clinic | Stored judgments; separate editorial observation of exact wording |
| Global B: Address → Cat only → Relevance → Opening hours | 0/3 COMPLIES; all continue toward booking, none mention cat-only | Stored judgments; separate editorial observation of exact wording |
